...
const reducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case 'EDIT_QUOTE_AUTHENTICITY':
      return {...state, quotes: state.quotes.map(q => {
        if (q.id != action.payload) return q
        return { ...q, apocryphal: !q.apocryphal}
      })}
    case 'SET_HIGHLIGHTED_QUOTE':
      return {...state,
        highlightedQuote: state.highlightedQuote === action.payload ? null : action.payload}
    case 'TOGGLE_VISIBILITY':
      return {...state, displayAll: !state.displayAll}
    default:
      return state
  }
}
  ...
  return (
    <div id="mp">
      ...
      // 👇 lots of props are missing! Check the Quotes component
        ...
        displayAllQuotes={state.displayAll}
